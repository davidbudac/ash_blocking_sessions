...
3. **Key-findings summary + charts**, each in its own `render*()` function:
   ...
   - `renderSwimlane()` — top-N waiters as a custom-series Gantt; runs of same-color buckets are coalesced into single bars. Container height grows with lane count (`interval: 0` on the y-axis — never let ECharts auto-skip labels, it makes lanes look dropped).
   - `renderTree()` / `drawTreeAt(idx)` — deterministic layered (tidy-tree) chain map at one bucket, scrubbable via `#treeScrub` (with `#treePlay`). Edges draw root blocker → waiter; **edge event labels show on hover only** (always-on labels collide at fan-in). Three node categories: blue "waiter", orange `#e67e22` "blocks & waits" (use the literal, not a CSS var, since canvas can't resolve it), red "root blocker". Node size ∝ √(downstream impact). **Clicking a node shows its own + all transitively downstream samples**, so a pure root blocker drills to its whole chain instead of "No samples". `XGAP` widens when identity toggles are active.
   - `renderChainTimeline()` — Gantt where each row is one SID, grouped by walking each waiter's *dominant parent* across the whole window to its ultimate root (cycle-guarded), then indented by depth. Filled cells = waiting that bucket; outlined cells = blocking others. Height grows with row count (`interval: 0`, same as the swimlane).
   - `renderSankey()` — blocker → waiter flow; grouping (`#sankeyGroup`) by session/user/module/program, capped by `#sankeyTopN`. Nodes are colored by role matching the tree (red root / orange mid / blue waiter); SID mode runs labels through `decorateSidLabel`.
4. **Cross-filter** (`applySelection(t)` / `highlightOverview()` / `updateSelectionBanner()`): module-level `activeData` (full set, or one focused `selectedT` sample) and `selectedT`. `renderTopRoots`, `renderTree` and `renderSankey` shadow `data` with `activeData` on their first line (`const data = activeData;`), so re-pointing `activeData` once filters all three. The two time-Gantts (`renderSwimlane`, `renderChainTimeline`) deliberately **always plot the full window** — a single focused sample would stretch into meaningless full-width bars — and instead draw a blue `markLine` at `selectedT`. `applySelection` also drives the `#sampleSel` banner (with `#sampleSelClear`), the `#sampleTableWrap` detail table (`showSamples()`), and the overview highlight band. `renderSummary` and `renderOverview` deliberately stay on the full dataset.
